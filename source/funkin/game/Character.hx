...
import funkin.psychstuff.PsychCharacter;
		...
		while(true) {
			switch (curCharacter)
			{
				...
				default:
					...
					switch (isPsych) {
						case true: 
							var jsonPath = Paths.json('characters/$curCharacter');
							if (!Assets.exists(jsonPath)) {
								curCharacter = "bf";
								continue;
							}
		
							var plainJSON = Assets.getText(jsonPath);

							var data:PsychCharacter = CoolUtil.parseJsonString(plainJSON);

							var cameraPosition:Array<Float> = [0, 0];
							var pposition:Array<Int> = [0, 0];
							var scale = data.scale;
							this.scale.set(scale, scale);
							updateHitbox();
							holdTime = data.sing_duration;
							cameraPosition = data.camera_position;
							cameraOffset.x = cameraPosition[0];
							cameraOffset.y = cameraPosition[1];
							flipX = !!data.flip_x;
							
							pposition = data.position;
							globalOffset.x = pposition[0];
							globalOffset.y = pposition[1];

							icon = data.healthicon;
							if(data.no_antialiasing) {
								antialiasing = false;
							}
							loadSprite(Paths.image(data.image));
							playerOffsets = isPlayer;

							var animationsArray:Array<PsychCharAnim> = [];
							...
							script = Script.create(Paths.script(Path.withoutExtension(jsonPath), null, true));
